function varargout = Articulated_2D(varargin)
% ARTICULATED_2D MATLAB code for Articulated_2D.fig
%      ARTICULATED_2D, by itself, creates a new ARTICULATED_2D or raises the existing
%      singleton*.
%
%      H = ARTICULATED_2D returns the handle to a new ARTICULATED_2D or the handle to
%      the existing singleton*.
%
%      ARTICULATED_2D('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in ARTICULATED_2D.M with the given input arguments.
%
%      ARTICULATED_2D('Property','Value',...) creates a new ARTICULATED_2D or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Articulated_2D_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Articulated_2D_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Articulated_2D

% Last Modified by GUIDE v2.5 03-Oct-2018 11:11:26

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Articulated_2D_OpeningFcn, ...
                   'gui_OutputFcn',  @Articulated_2D_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Articulated_2D is made visible.
function Articulated_2D_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Articulated_2D (see VARARGIN)

% Choose default command line output for Articulated_2D
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Articulated_2D wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Articulated_2D_OutputFcn(hObject, eventdata, handles) 
